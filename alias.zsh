...
#Get to my project - replace with a link to a template in your project!
...
###########################################################
#1. Hash 256 - output SHA-256 given any single input
#hash <email>
hash () {echo -n "$1" | shasum -a 256 | awk '{print $1}'}

#2. Hash your email - can optionally enter number for alias
#self
self () {if [ $# -eq 0 ]; then
...
#3. boss url to app url
#b2a <boss link>
b2a () {echo -n "$1\n" | sed 's/boss.prd-itbl.co/app.iterable.com/g'}

#4. app url to boss url
#a2b <app link>
...
#8. write simple E2 queries
e2 () {
  bash ~/bin/e2.zsh
}

#9. write simple Kubes queries
kube () {
  bash ~/bin/kube.zsh
}

#10. write simple ZD queries
zd () {
  bash ~/bin/zd.zsh
}
